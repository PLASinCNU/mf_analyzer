...
#include "llvm/IR/Module.h"  //ADD Here Rete 2019.04.29
#include "llvm/IR/IntrinsicInst.h"
#include "llvm/IR/Instructions.h"
#include "llvm/IR/Instruction.h" // Add Here Rete 2019.04.18
#include "llvm/Analysis/Interval.h"
#include "llvm/IR/InstrTypes.h"
#include "llvm/IR/User.h"; // Add Here Rete 2019.04.29
...
namespace {
//    struct testModule : public ModulePass { //BasicBlockPass
    struct testModule : public CallGraphSCCPass { //Module Pass
        static char ID;     // Pass identification
//        testModule(): ModulePass(ID) { }
        testModule(): CallGraphSCCPass(ID) { }
    ...
    private:  void eval_USEDEF(BasicBlock * bb) {
        ...
        for (BasicBlock::iterator inst = bb->begin(); inst != bb->end(); inst++) {
            ...
            if(isa<CallInst>(&bbInst)){
                ...
                Function *fn =cl->getCalledFunction();
//                errs() << "ADDRESS : " << &bbInst << "\n";
...
//                errs() << "Storrrreeee!  : "<< bbInst.getOperand(1)->getName() << "\n" ;
    ...
    private: bool isScanf(Function *fn){
    ...
    private:  void printTable(map<pair<Instruction *, string>, Instruction *> table){
        ...
        for(iter = table.begin() ; iter != table.end() ; iter++ ){
            ...
            string temp = "";
            //Add Here Rete 2019.04.29
            if(isa<CallInst>(iter->second)){
                CallInst *cl = cast<CallInst>(iter->second);
                string calledFunctionName = cl->getCalledFunction()->getName();
                Function *fn =cl->getCalledFunction();
                if(isScanf(fn)){
                    temp = cl->getCalledFunction()->getName();
                }
...
//            errs() << "\n\n heeeeeeeyyyyyyyyy!!!!!! " << temp << "      \n\n";
        ...
        private : string outputAnaly(string val, unsigned defLine){
            if(val.find("scanf") != std::string::npos){
                return "Relevant User Input";
            }
            else if(val.find("addr") != std::string::npos){
                return "Relevant Function Formal Parameter";
            }
...
char testModule::ID = 1;
static RegisterPass<testModule> X("testModule", "LV Pass");
